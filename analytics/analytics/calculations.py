...
def calculate_analyticity(student_id, test_id):
    ...
    analyticity = convert_analyticity_to_five_point_scale(
        round(analyticity_percentage, 2)
    )
...
def calculate_leadership(id_student):
    ...
    for result in results:
        ...
        for question in questions:
            solutions = tests_models.Solutions.objects.filter(
                id_result=result, id_question=question
            )
            ...
            correct_responses = sum(
                ...
                == solution.id_question.answers.get(is_correct=True).answer_text
            )
    ...
    if not response_density:
        ...
        iqr = round(
            np.percentile(response_density, 75) - np.percentile(response_density, 25), 2
        )
