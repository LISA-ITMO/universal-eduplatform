...
const AddTheme = () => {
  ...
  const handleSubmit = (e) => {
    ...
    API_SUBJECTS.themes
      ...
      .then((res) => {
        toast({
          title: `По предмету ${subjects.find((item) => item?.id == subjectId)?.name_subject} создана новая тема: ${theme}`,
          status: "success",
        });
        setSubjectId("");
        setTheme("");
      ...
      .catch((err) => {
        toast({
          title: `Не удалось создать новую тему, пожалуйста попробуйте позже`,
          status: "error",
        });
      });
  };
