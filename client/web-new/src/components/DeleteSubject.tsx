...
const DeleteSubject = () => {
  ...
  const handleSubmit = (e) => {
    ...
    API_SUBJECTS.subjects
      ...
      .then((res) => {
        toast({
          title: `Предмет ${subjects.find((item) => item?.id == id)?.name_subject} удален`,
          status: "success",
        });
        setId("");
        loadSubjectList();
      ...
      .catch((err) => {
        toast({
          title: `Предмет ${subjects.find((item) => item?.id == id)?.name_subject} не удалось удалить, пожалуйста попробуйте позже`,
          status: "error",
        });
      });
  };
